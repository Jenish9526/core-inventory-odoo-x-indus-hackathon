...
async function seed() {
  ...
  // Assign stock tiers: ~10% out of stock, ~15% low stock, rest healthy
  const productSkus = Object.keys(productMap);
  const shuffled = [...productSkus].sort(() => Math.random() - 0.5);
  const outOfStockCount = Math.floor(shuffled.length * 0.10);
  const lowStockCount = Math.floor(shuffled.length * 0.15);
  ...
  for (const [sku, { product, initialQty, reorderLevel }] of Object.entries(productMap)) {
    let mainQty, secQty, prodQty;

    if (outOfStockSkus.has(sku)) {
      // Out of stock: 0 in main, maybe tiny amount elsewhere
      mainQty = 0;
      secQty = rand(0, 1) === 0 ? 0 : rand(1, 3);
      prodQty = 0;
    } else if (lowStockSkus.has(sku)) {
      // Low stock: below reorder level
      const lowTotal = rand(1, Math.max(1, Math.floor(reorderLevel * 0.7)));
      mainQty = Math.round(lowTotal * 0.7);
      secQty = lowTotal - mainQty;
      prodQty = 0;
    } else {
      // Healthy stock: 2-5x reorder level, varied
      const healthyQty = Math.max(initialQty, reorderLevel * rand(2, 5));
      mainQty = Math.round(healthyQty * 0.65);
      secQty = Math.round(healthyQty * 0.25);
      prodQty = Math.round(healthyQty * 0.10);
    }
  ...
  console.log(`✅ Stock records: ${stockDocs.length} (${outOfStockCount} out of stock, ${lowStockCount} low stock)`);
